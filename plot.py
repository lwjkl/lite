...
def load_faiss_and_metadata(index_path, metadata_path, num_embeddings=-1):
...
def plot_matplotlib(standard_embedding, labels, image_paths, examples_per_cluster=5):
...
def figures_to_png_bytes(figures, dpi=300):
...
async def plot_embeddings(request):
    ...
    embeddings, _, image_paths = load_faiss_and_metadata(index_path, metadata_path)
    standard_embedding, labels = compute_umap_and_hdbscan(embeddings)

    fig_scatter, fig_examples = plot_matplotlib(
        standard_embedding, labels, image_paths, examples_per_cluster
    )

    png_bytes_list = figures_to_png_bytes([fig_scatter, fig_examples])
